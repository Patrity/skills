...
- **Nuxt 4 layout:** app code lives under `{{appDir}}/` (the `srcDir`) — `composables/`, `components/`, `pages/`, `layouts/`, `lib/`, `assets/`. Server/Nitro routes live under `server/`. Follow the existing structure rather than inventing new top-level dirs.
