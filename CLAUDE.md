...
- Nuxt UI is only for tree, select, input, form field, checkbox, radio, switch, tabs, tooltip, slide-over, toast host and buttons/icons. Everything else is Tailwind on the tokens above.
...
- One `<h1>` per page, always from our own markup: README and docs bodies have their leading `# Title` dropped server-side by `dropLeadingH1` (`server/utils/markdown.ts`).
