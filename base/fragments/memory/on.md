## Memory
- Use memory religiously: always reach for the `mymind` MCP server. Its memories come from every Claude Code session and are project-scoped: search them (`search_memories`, `search_docs`, `search_passages`) before answering from recollection and whenever you start discovery or a new implementation.
