...
It lists `.claude/settings.local.json`, `.claude/.env` when any installed bundle declares variables, and whatever paths the bundles themselves declare. A cache directory a skill writes into is the usual one. Every run regenerates the block from the current selection; lines outside it are yours and are never touched, and the block is removed once nothing needs it.
