...
## Every heading has exactly one home
...
| --- | --- | --- |
| `{{pm}}` | `pnpm`, `npm`, `yarn`, `bun` | commands, as in `{{pm}} test` |
| `{{pmx}}` | `pnpx`, `npx`, `yarn dlx`, `bunx` | one-off runners, as in `{{pmx}} tsx scripts/x.ts` |
| `{{appDir}}` | `app`, or the `appDir` answer (`apps/web/app`) | app-code globs, as in `{{appDir}}/**/*.vue` |
| `{{pkgDir}}` | nothing in a single-app repo; the package root with a trailing slash (`apps/web/`) in a monorepo | every other path inside the package, as in `{{pkgDir}}server/**` and `{{pkgDir}}nuxt.config.ts` |
...
repo root (`.claude/**`, CI config) unprefixed, because those do not move. A text-input axis contributes a
placeholder of its own id as well, so `appDir` is both an answer and `{{appDir}}`.

## One answer, one fragment

A fragment is a plain markdown file under `base/fragments/<axis>/<option>.md`. It carries the rules for exactly one answer, so picking a different option swaps the whole block cleanly.

```md
## Commands
- Always `pnpm`. If the project has a `package-lock.json` use npm; a `yarn.lock` means yarn.
...
- `fragment`: the file under `base/fragments/` this option contributes. An option with no fragment (`none`, `off`) contributes nothing.
...
## A profile is a set of answers with a name

Answers plus bundles, nothing else. One file per profile under `profiles/<name>.yaml`, with `name` matching the file name.
...
The wizard offers profiles first and pre-fills every answer from the one you pick; you can still change any of them. Non-interactively that is `--profile library`, with `--answer <axis>=<option>` for the overrides. Quote YAML values like `on`, `off`, `no` and `yes`. Unquoted they parse as booleans and stop matching an option id.
...
- Every `rules/*.md` in a bundle has a non-empty `paths:` list, because a rule with no globs never loads.
...
## How a bundle files itself under your headings
